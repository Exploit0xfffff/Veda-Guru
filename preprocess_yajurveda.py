...
output_dir = "./preprocessed_yajurveda"
...
def remove_metadata(text):
    ...
    text = re.sub(r'Next: Kanda \d+', '', text)
    return text.strip()
...
def main():
    for i in range(1, 8):
        input_file = os.path.join(input_dir, f"yajurveda_kanda_{i}.txt")
        output_file = os.path.join(output_dir, f"yajurveda_kanda_{i}.txt")
        preprocessed_text = preprocess_text(input_file)
        save_text(preprocessed_text, output_file)
        print(f"Kanda {i} preprocessed and saved.")
